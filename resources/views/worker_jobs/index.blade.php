...
@push('scripts')
    ...
    <script>
        $(document).ready(function() {
            ...
            // Diagnostics for JSuites dropdown mobile/touch blank issue
            setInterval(function() {
                var searchbar = document.querySelector('.jdropdown-searchbar');
                if (searchbar) {
                    var items = searchbar.querySelectorAll('.jdropdown-item');
                    var container = searchbar.querySelector('.jdropdown-container');
                    var content = searchbar.querySelector('.jdropdown-content');
                    
                    var debugDiv = document.getElementById('dropdown-debug');
                    if (!debugDiv) {
                        debugDiv = document.createElement('div');
                        debugDiv.id = 'dropdown-debug';
                    ...
                    var compSearchbar = window.getComputedStyle(searchbar);
                    var compContainer = container ? window.getComputedStyle(container) : null;
                    var compContent = content ? window.getComputedStyle(content) : null;
                    
                    debugDiv.innerHTML = `
                        <b>Dropdown Active Diagnostics</b><br/>
                        Searchbar display: ${compSearchbar.display}<br/>
                        Searchbar visibility: ${compSearchbar.visibility}<br/>
                        Searchbar height: ${searchbar.offsetHeight}px<br/>
                        Searchbar opacity: ${compSearchbar.opacity}<br/>
                        Searchbar rect: ${JSON.stringify(searchbar.getBoundingClientRect())}<br/>
                        <hr style="border-color:#444; margin: 5px 0;"/>
                        Container display: ${compContainer ? compContainer.display : 'N/A'}<br/>
                        Container height: ${container ? container.offsetHeight : 0}px<br/>
                        Content display: ${compContent ? compContent.display : 'N/A'}<br/>
                        Content height: ${content ? content.offsetHeight : 0}px<br/>
                        Items count: ${items.length}<br/>
                        Items display/colors: ${items.length > 0 ? window.getComputedStyle(items[0]).display + ' | ' + window.getComputedStyle(items[0]).color + ' | bg:' + window.getComputedStyle(items[0]).backgroundColor : 'N/A'}<br/>
                        Items HTML sample: ${items.length > 0 ? escapeHtml(items[0].outerHTML.substring(0, 100)) : 'N/A'}<br/>
                    `;
                } else {
                    var debugDiv = document.getElementById('dropdown-debug');
                    if (debugDiv) {
                        debugDiv.innerHTML = '<b>Dropdown inactive</b>';
                    }
                }
            }, 500);
            ...
            window.addEventListener('error', function(e) {
                ...
                errDiv.innerText = `ERR: ${e.message} at ${e.filename}:${e.lineno}`;
                debugDiv.appendChild(errDiv);
            });
        });
    </script>
@endpush
