...
@section('title', 'Edit Rencana Pertanian')
@section('page_title', 'Edit Rencana Pertanian')

@section('content')
<form action="{{ route('pertanians.update', $pertanian) }}" method="POST" id="form_pertanian">
    @csrf
    @method('PUT')
                ...
                <div class="card-body">
                        ...
                        <div class="col-md-6">
                            ...
                            <select name="kebun_id" class="form-select form-select-solid" data-control="select2" data-placeholder="Pilih kebun..." required>
                                ...
                                @foreach($kebuns as $kebun)
                                    <option value="{{ $kebun->id }}" {{ old('kebun_id', $pertanian->kebun_id) == $kebun->id ? 'selected' : '' }}>{{ $kebun->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="required form-label">Nama Rencana</label>
                            <input type="text" name="name" class="form-control form-control-solid" placeholder="Contoh: Proyek Cabai Musim Kemarau" value="{{ old('name', $pertanian->name) }}" required />
                        </div>
                    </div>
                    <div class="row mb-8">
                        <div class="col-md-6">
                            ...
                            <select name="admin_id" class="form-select form-select-solid" data-control="select2" data-placeholder="Pilih Admin" required>
                                <option></option>
                                @foreach($admins as $u)
                                    <option value="{{ $u->id }}" {{ old('admin_id', $pertanian->admin_id) == $u->id ? 'selected' : '' }}>{{ $u->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="row mb-8">
                        <div class="col-md-4">
                            ...
                            <input type="text" name="start_date" class="form-control form-control-solid kt_datepicker" placeholder="Pilih tanggal" value="{{ old('start_date', $pertanian->start_date) }}" />
                        </div>
                        <div class="col-md-4">
                            <label class="form-label">Tanggal Selesai</label>
                            <input type="text" name="end_date" class="form-control form-control-solid kt_datepicker" placeholder="Pilih tanggal" value="{{ old('end_date', $pertanian->end_date) }}" />
                        </div>
                        <div class="col-md-4">
                            <label class="required form-label">Status</label>
                            <select name="status" class="form-select form-select-solid" required>
                                <option value="Draft" {{ old('status', $pertanian->status) == 'Draft' ? 'selected' : '' }}>Draft</option>
                                <option value="Pencarian Investor" {{ old('status', $pertanian->status) == 'Pencarian Investor' ? 'selected' : '' }}>Pencarian Investor</option>
                                <option value="Sedang Berjalan" {{ old('status', $pertanian->status) == 'Sedang Berjalan' ? 'selected' : '' }}>Sedang Berjalan</option>
                                <option value="Selesai" {{ old('status', $pertanian->status) == 'Selesai' ? 'selected' : '' }}>Selesai</option>
                            </select>
                        </div>
                    </div>
                </div>
                                        ...
                                        <div class="col-md-3">
                                            ...
                                            <select name="tanaman_id" class="form-select form-select-solid tanaman-select2" data-placeholder="Pilih atau Ketik Baru...">
                                                ...
                                                @foreach($tanamans as $tanaman)
                                                    <option value="{{ $tanaman->id }}" {{ $pt->tanaman_id == $tanaman->id ? 'selected' : '' }}>{{ $tanaman->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-md-2">
                                            <label class="form-label d-md-none">Jumlah Pohon/Bibit</label>
                                            <input type="text" name="qty_pohon" class="form-control form-control-solid" placeholder="0" value="{{ $pt->qty_pohon }}" />
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label d-md-none">Est. Panen / Pohon (Kg)</label>
                                            <input type="text" name="estimasi_berat_per_pohon" class="form-control form-control-solid mask-decimal" placeholder="0.00" value="{{ $pt->estimasi_berat_per_pohon }}" />
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label d-md-none">Est. Harga / Kg (Rp)</label>
                                            <input type="text" name="estimasi_harga_per_kg" class="form-control form-control-solid mask-currency" placeholder="0" value="{{ $pt->estimasi_harga_per_kg }}" />
                                        </div>
                                ...
                                <div data-repeater-item class="mb-3">
                                    <div class="row g-3 align-items-center">
                                        <div class="col-md-4">
                                            ...
                                            <input type="text" name="name" class="form-control form-control-solid" placeholder="Contoh: Pupuk Urea, Gaji Tenaga" value="{{ $biaya->name }}" />
                                        </div>
                                        <div class="col-md-2">
                                            <label class="form-label d-md-none">Qty</label>
                                            <input type="text" name="qty" class="form-control form-control-solid biaya-qty" placeholder="1" value="{{ $biaya->qty }}" />
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label d-md-none">Harga Satuan (Rp)</label>
                                            <input type="text" name="harga_satuan" class="form-control form-control-solid mask-currency biaya-harga" placeholder="0" value="{{ $biaya->harga_satuan }}" />
                                        </div>
                                        <div class="col-md-2">
                                            <label class="form-label d-md-none">Total (Rp)</label>
                                            <input type="text" name="total" class="form-control form-control-transparent biaya-total" readonly placeholder="0" value="{{ $biaya->total }}" />
                                        </div>
                                        <div class="col-md-1 text-center">
                                            <label class="form-label d-md-none">Aksi</label>
                                            <div>
                ...
                <div class="card-body">
                    ...
                    <div class="row mb-8">
                        <div class="col-md-6">
                            ...
                            <input type="text" name="batasan_investasi" id="batasan_investasi" class="form-control form-control-solid mask-currency" placeholder="0" value="{{ old('batasan_investasi', $pertanian->batasan_investasi) }}" />
                    ...
                    <div class="row mb-8">
                        <div class="col-md-3">
                            ...
                            <input type="text" name="persentase_zakat" id="persen_zakat" class="form-control form-control-solid mask-decimal" placeholder="5.00" value="{{ old('persentase_zakat', $pertanian->persentase_zakat) }}" required />
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Investor (%)</label>
                            <input type="text" name="persentase_investor" id="persen_investor" class="form-control form-control-solid persen-input" placeholder="0" value="{{ old('persentase_investor', $pertanian->persentase_investor) }}" />
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Pengelola Lahan (%)</label>
                            <input type="text" name="persentase_pengelola" id="persen_pengelola" class="form-control form-control-solid persen-input" placeholder="0" value="{{ old('persentase_pengelola', $pertanian->persentase_pengelola) }}" />
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Admin (%)</label>
                            <input type="text" name="persentase_admin" id="persen_admin" class="form-control form-control-solid persen-input" placeholder="0" value="{{ old('persentase_admin', $pertanian->persentase_admin) }}" />
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-md-12">
...
<script>
    $(document).ready(function () {
        ...
        function calculateSummary() {
            // Estimasi Pendapatan from tanaman rows
            ...
            // Total Kebutuhan Modal from biaya rows
            let totalBiaya = 0;
        ...
        // Bind summary recalculation to all relevant inputs
