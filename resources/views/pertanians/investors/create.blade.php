...
@section('title', 'Tambah Investor - ' . $pertanian->name)
@section('page_title', 'Tambah Investor')

@section('content')
<div class="card">
    <div class="card-header">
        <h3 class="card-title">Form Tambah Investor untuk: <span class="text-primary">{{ $pertanian->name }}</span></h3>
    </div>
    
    <div class="card-body">
        <form action="{{ route('pertanians.investors.store', $pertanian) }}" method="POST" id="form_investor">
            @csrf
            ...
            <div class="mb-10">
                ...
                <select name="status" class="form-select form-select-solid" required>
                    <option value="Deal" {{ old('status') == 'Deal' ? 'selected' : '' }}>Deal (Selesai, masuk total)</option>
                    <option value="Standby" {{ old('status') == 'Standby' ? 'selected' : '' }}>Standby (Penjamin cadangan, masuk total)</option>
                    <option value="Nego" {{ old('status') == 'Nego' ? 'selected' : '' }}>Nego (Belum final, tidak masuk total)</option>
                    <option value="Batal" {{ old('status') == 'Batal' ? 'selected' : '' }}>Batal (Dibatalkan, tidak masuk total)</option>
                </select>
                @error('status')
                    <div class="text-danger mt-2">{{ $message }}</div>
                @enderror
            ...
            <div class="mb-10">
                ...
                <textarea name="keterangan" class="form-control form-control-solid" rows="3" placeholder="Masukkan keterangan tambahan jika ada">{{ old('keterangan') }}</textarea>
                @error('keterangan')
                    <div class="text-danger mt-2">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="d-flex justify-content-end">
                ...
                <button type="submit" class="btn btn-primary" id="btn_submit">Simpan Investor</button>
            </div>
        </form>
    </div>
</div>
    ...
    $(document).ready(function () {
        ...
        $('#form_investor').on('submit', function (e) {
            ...
            $.ajax({
                ...
                error: function (xhr) {
                    btn.prop('disabled', false).text('Simpan Investor');
