...
<!-- Tabs Navigation -->
<div class="tab-nav">
    <div class="tab-btn active" onclick="switchTab('rab')">Estimasi</div>
    <div class="tab-btn" onclick="switchTab('realisasi')">Realisasi</div>
    <div class="tab-btn" onclick="switchTab('withdrawals')">Penarikan</div>
...
<div class="content-area" style="padding-top: 0;">
    ...
    <!-- TAB 1: RAB & Estimasi -->
    <div id="tab-rab" class="tab-content active">
        <!-- Informasi Modal Investor -->
        <div class="investment-card">
        <h3 class="section-title" style="margin-bottom: 15px; font-size: 1.05rem;">Investasi Anda</h3>
        <div class="card-body" style="grid-template-columns: 1fr;">
            <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 10px; margin-bottom: 10px;">
                <span style="color: var(--text-muted); font-size: 0.9rem;">Batas Investasi Maksimal</span>
                <span style="font-weight: 600;">Rp {{ number_format($batasanInvestasi, 0, ',', '.') }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 10px; margin-bottom: 10px;">
                <span style="color: var(--text-muted); font-size: 0.9rem;">Modal Disetor</span>
                <span style="font-weight: 700;">Rp {{ number_format($inv->besaran_investasi, 0, ',', '.') }} <span style="font-size:0.75rem; color: var(--primary); font-weight: 500;">({{ number_format($invPercentage, 2) }}%)</span></span>
            </div>
            <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 10px; margin-bottom: 10px;">
                <span style="color: var(--text-muted); font-size: 0.9rem;">Bagi Hasil Saat Ini</span>
                <span class="{{ $inv->user_profit >= 0 ? 'success' : 'danger' }}" style="font-weight: 700;">
                    {{ $inv->user_profit >= 0 ? '+' : '' }}Rp {{ number_format($inv->user_profit, 0, ',', '.') }}
                </span>
            </div>
            <div style="display: flex; justify-content: space-between;">
                <span style="color: var(--text-muted); font-size: 0.9rem;">Estimasi Akhir</span>
                <span class="{{ $inv->estimasi_user_profit >= 0 ? 'success' : 'danger' }}" style="font-weight: 700;">
    ...
    <!-- Estimasi Keseluruhan Proyek -->
    <div class="investment-card">
        <h3 class="section-title" style="margin-bottom: 15px; font-size: 1.05rem;">RAB & Estimasi Total (Proyek)</h3>
        <div class="card-body" style="grid-template-columns: 1fr;">
            <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 10px; margin-bottom: 10px;">
                <span style="color: var(--text-muted); font-size: 0.9rem;">Total Estimasi Biaya</span>
                <span style="font-weight: 600; color: var(--danger);">Rp {{ number_format($estimasiBiaya, 0, ',', '.') }}</span>
            </div>
            <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 10px; margin-bottom: 10px;">
                <span style="color: var(--text-muted); font-size: 0.9rem;">Total Estimasi Panen</span>
                <span style="font-weight: 600; color: var(--primary-dark);">Rp {{ number_format($estimasiPendapatan, 0, ',', '.') }}</span>
            </div>
            <div style="display: flex; justify-content: space-between;">
                <span style="color: var(--text-muted); font-size: 0.9rem;">Estimasi Laba Kotor</span>
                <span style="font-weight: 700; color: {{ $estimasiLaba >= 0 ? 'var(--primary-dark)' : 'var(--danger)' }};">
    ...
    <div id="tab-info" class="tab-content">
        <div class="investment-card">
            ...
            <div class="card-body">
                ...
                <div class="data-group" style="margin-top: 15px; border-top: 1px solid rgba(0,0,0,0.05); padding-top: 15px;">
    ...
    <!-- TAB 2: Realisasi -->
    <div id="tab-realisasi" class="tab-content">
    <!-- Transaksi Aktual Keseluruhan Proyek -->
    <div class="investment-card">
        <h3 class="section-title" style="margin-bottom: 15px; font-size: 1.05rem;">Realisasi Transaksi (Proyek)</h3>
        
        <details>
            <summary>
                <span>Pengeluaran Barang</span>
                <span style="color: var(--danger);">Rp {{ number_format($totalPurchase, 0, ',', '.') }}</span>
            </summary>
            <div style="margin-top: 10px; padding-top: 5px; border-top: 1px solid rgba(0,0,0,0.05);">
                @forelse($pertanian->purchases as $purchase)
                    @foreach($purchase->items as $item)
                    <div class="detail-item">
                        <span style="color: var(--text-muted);">{{ $item->name }} ({{ $item->qty }}x)</span>
                        <span style="font-weight: 500;">Rp {{ number_format($item->total_price, 0, ',', '.') }}</span>
                    </div>
                    @endforeach
                @empty
                    <div class="detail-item" style="color: var(--text-muted); justify-content: center; font-style: italic;">Belum ada pengeluaran</div>
                @endforelse
            </div>
        </details>
        ...
        <details>
            <summary>
                <span>Pengeluaran Upah Pekerja</span>
                <span style="color: var(--danger);">Rp {{ number_format($totalWorker, 0, ',', '.') }}</span>
            </summary>
            <div style="margin-top: 10px; padding-top: 5px; border-top: 1px solid rgba(0,0,0,0.05);">
                @forelse($pertanian->workerJobs as $job)
                    <div class="detail-item">
                        <span style="color: var(--text-muted);">{{ \Carbon\Carbon::parse($job->date)->format('d M') }} - Pekerja</span>
                        <span style="font-weight: 500;">Rp {{ number_format($job->wage, 0, ',', '.') }}</span>
                    </div>
                @empty
                    <div class="detail-item" style="color: var(--text-muted); justify-content: center; font-style: italic;">Belum ada upah pekerja</div>
                @endforelse
            </div>
        </details>
        ...
        <details>
            <summary>
                ...
                <span style="color: var(--primary-dark);">Rp {{ number_format($totalIncome, 0, ',', '.') }}</span>
            </summary>
            <div style="margin-top: 10px; padding-top: 5px; border-top: 1px solid rgba(0,0,0,0.05);">
                @forelse($pertanian->incomes as $income)
                    <div class="detail-item">
                        <span style="color: var(--text-muted);">{{ \Carbon\Carbon::parse($income->date)->format('d M') }} - {{ $income->description ?? 'Penjualan' }}</span>
                        <span style="font-weight: 500;">Rp {{ number_format($income->nominal, 0, ',', '.') }}</span>
                    </div>
                @empty
                    <div class="detail-item" style="color: var(--text-muted); justify-content: center; font-style: italic;">Belum ada pendapatan</div>
                @endforelse
            </div>
        </details>
        ...
        <div class="card-body" style="grid-template-columns: 1fr; margin-top: 15px; padding-top: 15px; border-top: 1px solid rgba(0,0,0,0.1);">
            ...
            <div style="margin-top: 20px; text-align: center;">
                <a href="{{ route('investor.pertanian.laporan', $pertanian->uuid) }}" target="_blank" style="display: block; background: var(--primary); color: white; padding: 10px; border-radius: 20px; text-decoration: none; font-weight: 600; font-size: 0.9rem; box-shadow: 0 4px 10px rgba(16, 185, 129, 0.3);">
    ...
    </div> <!-- End Tab Realisasi -->

    <!-- TAB 3: Penarikan (Withdrawals) -->
    <div id="tab-withdrawals" class="tab-content">
        <div class="investment-card">
            <h3 class="section-title" style="margin-bottom: 15px; font-size: 1.05rem;">Penarikan Dana Bagi Hasil</h3>
            <div class="card-body" style="grid-template-columns: 1fr;">
                <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 10px; margin-bottom: 10px;">
                    <span style="color: var(--text-muted); font-size: 0.9rem;">Total Hak Bagi Hasil Anda</span>
                    <span style="font-weight: 600;">Rp {{ number_format($inv->user_profit, 0, ',', '.') }}</span>
                </div>
                <div style="display: flex; justify-content: space-between; border-bottom: 1px solid rgba(0,0,0,0.05); padding-bottom: 10px; margin-bottom: 10px;">
                    <span style="color: var(--text-muted); font-size: 0.9rem;">Total Sudah Ditarik</span>
                ...
                <div style="display: flex; justify-content: space-between;">
                    ...
                    <span style="font-weight: 700; color: var(--success); font-size: 1.1rem;">Rp {{ number_format($sisaBisaDitarik, 0, ',', '.') }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="investment-card">
            ...
            <div style="border: 1px solid rgba(0,0,0,0.05); border-radius: 12px; padding: 15px; margin-bottom: 10px; background: white;">
                ...
                <div style="font-size: 0.85rem; color: var(--text-muted); margin-bottom: 8px;">
                    <i class="ki-duotone ki-text-align-left me-1"></i> {{ $withdrawal->notes }}
                </div>
                @endif
                ...
                <div style="margin-top: 10px;">
                    <a href="{{ Storage::url($withdrawal->proof_image) }}" target="_blank" style="font-size: 0.8rem; text-decoration: none; background: var(--light); padding: 4px 10px; border-radius: 10px; color: var(--primary); display: inline-flex; align-items: center;">
                        <i class="ki-duotone ki-picture me-1"></i> Lihat Bukti Transfer
                    </a>
                </div>
                @endif
            </div>
        ...
        @else
            <div class="timeline">
                ...
                <div class="timeline-item">
                    ...
                    <div class="timeline-card investment-card">
                        ...
                        @if(is_array($update->photo) && count($update->photo) > 0)
                            ...
                            @foreach($update->photo as $img)
                                <div style="flex: 1 1 calc(50% - 4px); min-width: 120px; cursor: pointer;" onclick="showLightbox('{{ Storage::disk('r2')->url($img) }}')">
                        ...
                        @elseif(is_string($update->photo) && $update->photo)
                            <div class="mb-4" style="cursor: pointer;" onclick="showLightbox('{{ Storage::disk('r2')->url($update->photo) }}')">
...
<script>
    function switchTab(tabId) {
        ...
        if(tabId === 'info' && typeof detailMap !== 'undefined') {
            setTimeout(function() {
...
@push('scripts')
    ...
    <script>
        ...
        window.showLightbox = function(src) {
            ...
            // Remove bootstrap classes since we don't have bootstrap here
